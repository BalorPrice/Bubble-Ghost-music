...
; Interrupts uses a linked list for multiple interrupts per frame
int.next:						dw 0					; Address of next entry in linked list
int.jump:						dw 0					; Address of current routine to run
int.reset:						dw 0					; Address of start of linked list
...
int.handler.src:
;=======================================================
; Interrupt handler is copied under screen
...
int.start:
	...
	int.set:
		@set_line:										; set line address of next routine
				...
				out (StatusReg),a
				inc hl
		@set_rout:										; set routine address for this interrupt
				ld c,(hl)
				inc hl
				ld b,(hl)
				ld (int.jump),bc
				inc hl
		@set_next:										; store address of next interrupt entry in table
				ld (int.next),hl
...
; Null list of just one entry
				db 42
				dw int.pal_light,int.ll.off + 5
				db 191 - 42
				dw int.pal_dark,int.ll.off + 10
				db -1
